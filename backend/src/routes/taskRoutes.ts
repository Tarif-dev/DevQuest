...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Auto-trigger Vincent AI analysis
      try {
          ...
          message:
            contributionStatus === "APPROVED"
              ? `🎉 Work submitted and approved! Score: ${
                  vincentResult.score
                }/100. Payout of ${payoutAmount.toFixed(2)} PYUSD is pending.`
              : `⚠️ Work submitted but needs improvement. Score: ${vincentResult.score}/100 (minimum 60 required). Check feedback for details.`,
        });
      } catch (analysisError) {
        // If Vincent AI fails, still return success for submission
        console.error("Vincent AI analysis failed:", analysisError);
        res.status(201).json({
          success: true,
          data: {
            contribution,
